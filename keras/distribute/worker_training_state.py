...
def _should_enable_preemption_checkpoint(preemption_checkpoint_arg, strategy):
...
class WorkerTrainingState:
    ...
    def __init__(
        ...
        self._enable_preemption_checkpoint = (
            _should_enable_preemption_checkpoint(
                preemption_checkpoint_arg, model.distribute_strategy
            )
        )
        self._model = model
    ...
    def maybe_load_initial_counters_from_ckpt(
        ...
        if mode == mode_keys.ModeKeys.TRAIN:
            ...
            if self._enable_preemption_checkpoint or isinstance(
                self._save_freq, int
            ):
                if batch >= 0:
